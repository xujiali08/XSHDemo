...
public extension LYProgressHUD {
    ...
    /**
     ...
     - parameter autoRemove: 是否自动移除,默认3秒后自动移除
     */
    public static func showEpLoading(_ text: String = "", autoRemove: Bool = true) {
        DispatchQueue.main.async {
            LYProgressHUD.showEpWaitingWithText(text: text, autoRemove: autoRemove)
        }
    }
...
private let windowBgColor = UIColor.black.withAlphaComponent(0.3)
...
//        window.center = getCenter()
    ...
    static func getCenter() -> CGPoint {
        guard let view = UIApplication.shared.keyWindow?.subviews.first else {
            return CGPoint(x: kScreenW/2.0, y: kScreenH/2.0)
        }
